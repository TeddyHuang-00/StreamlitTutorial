...
st.write("你可以在编辑器里看到实时运行的结果！")
table_data = {"A": [1, 2], "B": [3, 4]}
st.write(pd.DataFrame(data=table_data))
"""

st.sidebar.radio("编辑器布局", ["垂直分屏", "水平分屏"], index=0, key="layout", horizontal=True)
...
with editor:
    ...
    code = st_ace(
        ...
        theme=st.sidebar.selectbox("编辑器主题", options=THEMES, index=22) or "monokai",
